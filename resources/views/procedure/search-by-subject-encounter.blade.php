...
<div class="container">
    ...
    <div class="dropdown show">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
            ...
            <a class="dropdown-item" href="{{route('composition.search-by-encounter')}}">Encounter</a>
        </div>
    </div>
    <br>
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('composition.search-by-subject-encounter') }}" method="GET">
                <div class="form-group">
                    <label for="title">Search by Subject and Encounter:</label>
                </div>
                <div class="d-flex">
                    <div class="form-group">
                        <label for="subject">Subject:</label>
                        <input type="text" class="form-control" id="subject" name="subject" placeholder="Enter composition subject">
                    </div>
                    <div class="form-group mx-4">
                        <label for="encounter">Encounter:</label>
                        <input type="text" class="form-control" id="encounter" name="encounter" placeholder="Enter composition encounter">
                    </div>
                </div>
                ...
                <table id="organizations-table" class="table display">
                    ...
                    <tbody>
                        @if (isset($composition))
                            @if (!empty($composition['entry']))
                                <?php $no = 1; ?>
                                @foreach ($composition['entry'] as $entry)
                                    <tr>
                                        <td>{{ $no++ }}</td>
