...
@endif
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('patient.search-by-id') }}" method="GET">
                ...
                <table id="organizations-table" class="table display">
                    ...
                    <tbody>
                        ...
                        @if(isset($patientId))
                            ...
                            <td>Active</td>
                            @elseif($patient['active'] == false)
                            <td>Inactive</td>
                            @else
                            <td>{{ $patient['active'] ?? 'N/A' }}</td>
                            @endif
                            <td>{{ $patient['identifier'][0]['use'] ?? 'N/A' }}</td>
                            <td>{{ $patient['identifier'][0]['value'] ?? 'N/A' }}</td>
                            @endif
                            @else
