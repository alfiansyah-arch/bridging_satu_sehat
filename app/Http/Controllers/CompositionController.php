...
class CompositionController extends Controller
{
    public function SearchId(Request $request)
    {
        ...
        $compositionId = $request->input('id');

        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Composition/' . $compositionId);

        $composition = $response->json();

        return view('composition.search-by-id', compact('composition', 'compositionId', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchSubject(Request $request)
    {
        ...
        $accessToken = AccessToken::find(1)->token;
        $accessTokenExpiry = AccessToken::find(1)->expired;
        $compositionSubject = $request->input('subject');

        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Composition?subject=' . $compositionSubject);

        $composition = $response->json();

        return view('composition.search-by-subject', compact('composition', 'compositionSubject', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchEncounter(Request $request)
    {
        ...
        $accessToken = AccessToken::find(1)->token;
        $accessTokenExpiry = AccessToken::find(1)->expired;
        $compositionEncounter = $request->input('encounter');

        if(empty($compositionEncounter)){
        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Composition?encounter=null');
        }else{
        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Composition?encounter=' . $compositionEncounter);
        }
        $composition = $response->json();

        return view('composition.search-by-encounter', compact('composition', 'compositionEncounter', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchSubjectEncounter(Request $request)
    {
        ...
        if(empty($compositionEncounter)){
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $accessToken
            ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?subject=' . $compositionSubject.'&encounter=null');
        }else{
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $accessToken
            ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?subject=' . $compositionSubject.'&encounter='.$compositionEncounter);
        }

        $composition = $response->json();

        return view('composition.search-by-subject-encounter', compact('composition', 'compositionSubject', 'compositionEncounter', 'accessToken', 'accessTokenExpiry'));
    }
