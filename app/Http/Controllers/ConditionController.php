...
class ConditionController extends Controller
{
    public function SearchSubject(Request $request)
    {
        ...
        $conditionSubject = $request->input('subject');

        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?subject=' . $conditionSubject);

        $condition = $response->json();

        return view('condition.search-by-subject', compact('condition', 'conditionSubject', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchEncounter(Request $request)
    {
        ...
        $accessToken = AccessToken::find(1)->token;
        $accessTokenExpiry = AccessToken::find(1)->expired;
        $conditionEncounter = $request->input('encounter');

        if(empty($conditionEncounter)){
        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?encounter=null');
        }else{
        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?encounter=' . $conditionEncounter);
        }
        $condition = $response->json();

        return view('condition.search-by-encounter', compact('condition', 'conditionEncounter', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchSubjectEncounter(Request $request)
    {
        ...
        if(empty($conditionEncounter)){
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $accessToken
            ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?subject=' . $conditionSubject.'&encounter=null');
        }else{
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $accessToken
            ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition?subject=' . $conditionSubject.'&encounter='.$conditionEncounter);
        }

        $condition = $response->json();

        return view('condition.search-by-subject-encounter', compact('condition', 'conditionSubject', 'conditionEncounter', 'accessToken', 'accessTokenExpiry'));
    }
    public function SearchId(Request $request)
    {
        ...
        $accessToken = AccessToken::find(1)->token;
        $accessTokenExpiry = AccessToken::find(1)->expired;
        $conditionId = $request->input('subject');

        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $accessToken
        ])->get(env('SATUSEHAT_FHIR_URL') . '/Condition/' . $conditionId);

        $condition = $response->json();

        return view('condition.search-by-id', compact('condition', 'conditionId', 'accessToken', 'accessTokenExpiry'));
    }
